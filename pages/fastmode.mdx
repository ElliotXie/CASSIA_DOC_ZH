...
总的来说，CASSIA的 注释agent 和 检查agent 首先会接收到marker文件，并互相沟通以得出初步的注释结果与逻辑推理，之后 评估agent 会对注释的结果进行打分，如果分数低于阈值的注释会被送到 注释增强agent 中继续分析，注释增强agent 可以阅读整个marker file而不是仅仅前50。这一个工作流结束之后，你会得到三份文件，分别是基本注释的报告，注释结果的表格，和注释增强的结果报告。用户也可以通过
